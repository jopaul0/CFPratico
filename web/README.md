...
* Exportação: xlsx (Excel), Geração de PDF nativa do Electron
...
├── contexts/         # Contextos globais (ModalContext, RefreshContext)
├── hooks/            # Hooks customizados com a lógica de negócio (useDashboardData, etc.)
├── navigation/       # Configuração do React Router (AppNavigatior)
...
│   ├── database/
│   │   └── crud/     # Funções Dexie (fetch, add, update, delete)
