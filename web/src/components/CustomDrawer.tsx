...
import { useUserConfig } from '../hooks/useUserConfig'; // <-- IMPORTADO

// ... (LinkItemProps e LinkItem permanecem iguais) ...
...
export const CustomDrawer: React.FC<CustomDrawerProps> = ({ isOpen, onClose }) => {
  // Pega o config do hook
  const { config } = useUserConfig(); // <-- ADICIONADO

  // Define a fonte da logo
  const logoSrc = config?.company_logo || '/icon.png'; // <-- ADICIONADO
  const companyName = config?.company_name || 'CF Prático'; // <-- ADICIONADO
